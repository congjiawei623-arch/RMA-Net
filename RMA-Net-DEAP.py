...
# Keep consistent with the DEAP preprocessing code referenced in the paper
num_classes = 2
batch_size = 64
...
SAVE_DIR = os.path.join(BASE_DIR, 'visualizations', 'RMA_Net')
...
CLASS_COLORS = [SCI_COLORS['coral'], SCI_COLORS['teal']]
...
DEAP_32_GRID_MAP = {
    'Fp1': (0, 3), 'Fp2': (0, 5), 'AF3': (1, 2), 'AF4': (1, 6),
    ...
    'O1': (7, 1), 'PO3': (7, 2), 'Oz': (7, 4), 'PO4': (7, 6), 'O2': (7, 7)
}

# Training hyperparameters (consistent with Table 5-1 in the paper)
LR = 1e-3
WEIGHT_DECAY = 1e-5
MAX_EPOCHS = 100
EARLY_STOP = 10
...
BLOCK_MISSING_PROB = 0.50      # probability of applying a block mask per sample
MAX_BLOCKS = 2                 # max number of block regions
BLOCK_SIZE_RANGE = (2, 4)      # block size on the 8x9 grid

# USE_SAVED_CACHE = True
# FORCE_RETRAIN = False
...
FULL_CACHE_PATH = os.path.join(CACHE_DIR, 'rma_net_deap_cache.pkl')


# ==================== Data Loading ====================
def load_data(subject, dataset_dir, flag='v'):
    """
    Load preprocessed DEAP data.
    flag='v' for valence, flag='a' for arousal.
    The data is expected to be in .mat files with keys:
        'data', 'valence_labels', 'arousal_labels'.
    Returns arrays of shape [samples, T=6, C=4, H=8, W=9].
    """
    file_path = os.path.join(dataset_dir, 'DE_s' + subject)
    file = sio.loadmat(file_path)
    data = file['data']
    y_v = file['valence_labels'][0]
    y_a = file['arousal_labels'][0]

    y_v = F.one_hot(torch.tensor(y_v, dtype=torch.long), num_classes=num_classes).float()
    y_a = F.one_hot(torch.tensor(y_a, dtype=torch.long), num_classes=num_classes).float()

    one_falx = data.transpose([0, 3, 1, 2])
    one_falx = one_falx.reshape((-1, num_windows, num_chan, img_rows, img_cols))

    one_y_v = torch.empty((0, num_classes))
    one_y_a = torch.empty((0, num_classes))
    for j in range(int(len(y_a) // num_windows)):
        one_y_v = torch.vstack((one_y_v, y_v[j * num_windows]))
        one_y_a = torch.vstack((one_y_a, y_a[j * num_windows]))

    one_y = one_y_v if flag == 'v' else one_y_a
    # one_falx is already a numpy array
    return one_falx.astype(np.float32), one_y.numpy().astype(np.float32)
...
def masked_mse(pred, target, missing_mask):
    """
    Reconstruction loss computed only over missing regions.
    missing_mask: 1 indicates a missing (occluded) position, 0 indicates valid.
    """
...
def generate_random_block_mask(batch_size, h, w, p_random_max=0.3,
    ...
    """
    Generate hybrid masks as described in Chapter 5:
    M = Mr * Mb.
    Returns mask_keep, where 1 means keep and 0 means missing.
    """
    masks = []
...
# ==================== Evaluation Masks (Chapter 5 experiments) ====================
DEAP_REGION_BLOCKS = {
    'frontal': [(0, 0), (4, 9)],
    'central': [(2, 0), (6, 9)],
...
def build_eval_mask(batch_size, h, w, mode='full', ratio=0.0, device='cpu', region='frontal', block_size='small'):
    """
    Create fixed masks for evaluation under incomplete inputs.
    - full: complete input
    - random: random missing with a given ratio (e.g., 0.1/0.2/0.3/0.5)
    - region: region-based block missing
    - block: small/medium/large block missing
    - channels8/channels16/channels24: reduced-channel conditions
    Returns mask_keep (1=keep, 0=missing).
    """
    if mode == 'full':
    ...
    for _ in range(batch_size):
        ...
        if mode == 'random':
            ...
            if region not in DEAP_REGION_BLOCKS:
                region = 'frontal'
            (y1, x1), (y2, x2) = DEAP_REGION_BLOCKS[region]
            m[y1:y2, x1:x2] = 0.0
        elif mode == 'block':
            if block_size not in DEAP_BLOCK_SPECS:
                block_size = 'small'
            ...
            raise ValueError(f'Unsupported eval mask mode: {mode}')
        masks.append(m)
    return torch.stack(masks, dim=0)


# ==================== RMA-Net Modules (Chapter 5) ====================
class ReliabilityEstimator(nn.Module):
    """
    Section 5.2.2:
    Q_t = [U_t, V_t, M_t] -> Conv3x3 -> Conv3x3 -> Conv1x1 -> R_t
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x_masked, mask_keep):
        # x_masked: [B, C, H, W], mask_keep: [B, H, W]
        u = x_masked.mean(dim=1, keepdim=True)                            # [B,1,H,W]
        v = (x_masked - u).abs().mean(dim=1, keepdim=True)                # [B,1,H,W]
        m = mask_keep.unsqueeze(1)                                        # [B,1,H,W]
        q = torch.cat([u, v, m], dim=1)                                   # [B,3,H,W]
        r = self.net(q)                                                   # [B,1,H,W]
        rho_t = r.mean(dim=(2, 3), keepdim=True)                          # [B,1,1]
        return r, rho_t


class ChannelReconstructor(nn.Module):
    """
    Section 5.2.3: Joint encoder-decoder reconstruction module.
    Y_t = [X~_t, R_t]
    """
    ...
    def forward(self, x_masked, r):
        y = torch.cat([x_masked, r], dim=1)      # [B,C+1,H,W]
        e1 = self.enc1(y)
        e2 = self.enc2(e1)
        z = self.enc3(e2)
...
class QualityGuidedFrequencyAttention(nn.Module):
    """
    Section 5.2.5 (part 1): Frequency attention.
    Uses reliability-weighted global statistics to generate band weights.
    """
    ...
    def forward(self, x_bar, r):
        # x_bar: [B,C,H,W], r: [B,1,H,W]
        g = (x_bar * r).mean(dim=(2, 3))             # [B,C]
        q = F.gelu(self.fc1(g))
        alpha = F.softmax(self.fc2(q), dim=1)        # [B,C]
...
class MultiScaleResidualBackbone(nn.Module):
    """
    Section 5.2.4:
    Initial 3x3 conv + 3 residual blocks.
    The last two blocks use stride=2, achieving 2x downsampling twice.
    Final feature map size is approx. 2x3 (downsampled from 8x9).
    """
...
class QualityGuidedSpatialAttention(nn.Module):
    """
    Section 5.2.5 (part 2): Spatial attention.
    P_st = [S_t, R'_t] -> Conv3x3 -> Conv3x3 -> Conv1x1 -> A_st
    """
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
...
class QualityGuidedTemporalAttention(nn.Module):
    """
    Section 5.2.5 (part 3): Temporal attention.
    Explicitly reshapes rho_seq to [B,T,1] to avoid broadcasting dimension issues.
    """
    ...
    def forward(self, h_seq, rho_seq):
        ...
        # Force rho_seq into [B,T,1]
        if rho_seq.dim() == 2:
            rho_seq = rho_seq.unsqueeze(-1)
        elif rho_seq.dim() == 3 and rho_seq.shape[1] == 1 and rho_seq.shape[2] == T:
            rho_seq = rho_seq.transpose(1, 2)
        elif rho_seq.dim() != 3:
            raise ValueError(f"rho_seq shape error: {rho_seq.shape}")

        rho_seq = rho_seq.contiguous().view(B, T, 1)
        rho_bar = rho_seq.mean(dim=1, keepdim=True)                           # [B,1,1]
        rho_tilde = rho_seq / (rho_bar + self.eps_rho)                        # [B,T,1]

        h_proj = self.W_h(h_seq)                                              # [B,T,d]
        r_proj = self.w_r(rho_tilde)                                          # [B,T,d]
        a = torch.tanh(h_proj + r_proj)                                       # [B,T,d]
        e = self.v(a).squeeze(-1)                                             # [B,T]
        alpha = F.softmax(e, dim=1)
        z = torch.sum(alpha.unsqueeze(-1) * h_seq, dim=1)                     # [B,128]
        return z, alpha


class RMANet(nn.Module):
    """
    Complete RMA-Net for DEAP (Chapter 5):
    Mask simulation -> Reliability estimation -> Channel reconstruction ->
    Frequency attention -> Residual backbone -> Spatial attention ->
    ...
    def forward_once(self, x, inject_mask=True, eval_mask_keep=None):
        ...
        if eval_mask_keep is not None:
            ...
            mask_keep = generate_random_block_mask(
                ...
                p_random_max=RANDOM_MISSING_MAX,
        ...
        mask_expand = mask_keep.unsqueeze(1).unsqueeze(1)                     # [B,1,1,H,W]
        ...
        for ti in range(t):
            x_t = x[:, ti]                                                   # [B,C,H,W]
            x_masked_t = x_masked[:, ti]                                     # [B,C,H,W]
            mask_t = mask_keep                                               # [B,H,W]

            r_t, rho_t = self.reliability(x_masked_t, mask_t)                # [B,1,H,W], [B,1,1]
            x_hat_t = self.reconstructor(x_masked_t, r_t)                    # [B,C,H,W]

            # Fuse: keep valid regions, compensate missing regions with reconstruction
            ...
            h_t = F.adaptive_avg_pool2d(x_s_t, output_size=1).flatten(1)     # [B,128]

            time_features.append(h_t)
            rho_seq.append(rho_t.view(rho_t.size(0), 1))                        # [B,1]
            recon_list.append(x_hat_t)
            reliab_list.append(r_t)
            band_weights.append(alpha_f_t)
            spatial_weights.append(a_s_t)

        h_seq = torch.stack(time_features, dim=1)                            # [B,T,128]
        rho_seq = torch.stack(rho_seq, dim=1)                                # [B,T,1]
        z, alpha_t = self.temporal_att(h_seq, rho_seq)
        logits = self.classifier(z)

        x_hat = torch.stack(recon_list, dim=1)                               # [B,T,C,H,W]
        r_all = torch.stack(reliab_list, dim=1)                              # [B,T,1,H,W]
        band_weights = torch.stack(band_weights, dim=1)                      # [B,T,C]
        spatial_weights = torch.stack(spatial_weights, dim=1)                # [B,T,1,H',W']
...
def get_class_names(n_classes):
    return ['Low Valence', 'High Valence'] if n_classes == 2 else [f'Class {i}' for i in range(n_classes)]


def channel_vector_to_grid(channel_values, fill_value=np.nan):
    grid = np.full((img_rows, img_cols), fill_value, dtype=np.float32)
    vals = np.asarray(channel_values, dtype=np.float32).reshape(-1)
    for idx, ch in enumerate(DEAP_CHANNEL_NAMES):
        r, c = DEAP_32_GRID_MAP[ch]
        if idx < len(vals):
            grid[r, c] = vals[idx]
    if np.isnan(grid).any():
        valid = np.isfinite(grid)
        grid[~valid] = np.nanmean(grid[valid]) if np.any(valid) else 0.0
    return grid


def grid_to_channel_vector(weight_map):
    grid = np.asarray(weight_map, dtype=np.float32)
    if grid.ndim != 2:
        grid = grid.reshape(img_rows, img_cols)
    channel_values = []
    for ch in DEAP_CHANNEL_NAMES:
        r, c = DEAP_32_GRID_MAP[ch]
        channel_values.append(float(grid[r, c]))
    return np.asarray(channel_values, dtype=np.float32)
...
def plot_stability_summary(all_stability, save_path):
    def _plot():
        ...
        plt.xticks(x, labels, rotation=20); plt.ylabel('Score'); plt.title('Chapter-5 Stability under Incomplete Inputs'); plt.grid(True, axis='y', alpha=0.2); plt.legend(); plt.tight_layout(); plt.savefig(save_path, dpi=180, bbox_inches='tight'); plt.close()
...
def plot_mask_examples(mask_examples, save_path):
    def _plot():
        ...
        for ax, item in zip(axes, mask_examples[:n]):
            ...
            ax.set_title('Keep Mask')
            ax.axis('off')
        plt.suptitle('Incomplete-input Mask Examples')
        plt.tight_layout(); plt.savefig(save_path, dpi=180, bbox_inches='tight'); plt.close()
...
def collect_attention_and_probs(model, loader):
    ...
    temporal_list, band_list, spatial_list = [], [], []
    mask_examples = []
    with torch.no_grad():
        for bi, (inputs, labels) in enumerate(loader):
            ...
            if out['spatial_attention'] is not None:
                s = out['spatial_attention'].mean(dim=(0,1,2)).cpu().numpy()
                spatial_list.append(s)
            if bi < 4: mask_examples.append(out['mask_keep'][0].cpu().numpy())
    temporal = np.vstack(temporal_list) if len(temporal_list) > 0 else None
    band = np.vstack(band_list).mean(axis=0) if len(band_list) > 0 else None
    spatial = np.mean(np.stack(spatial_list, axis=0), axis=0) if len(spatial_list) > 0 else None
...
def compute_total_loss(model, x, labels, training=True):
    """
    Joint loss described in Chapter 5:
    L = L_cls + lambda1 * L_rec + lambda2 * L_cons + lambda3 * L_reg

    1) L_cls: cross-entropy on masked inputs
    2) L_rec: reconstruction MSE only over missing regions
    3) L_cons: KL-divergence between predictions from full and masked inputs
    4) L_reg: lightweight regularization for reliability maps vs. mask prior
    """
    ...
    # Reconstruction loss: only on missing regions
    missing_mask = (1.0 - mask_keep).unsqueeze(1).unsqueeze(1)              # [B,1,1,H,W]
...
def train_one_fold(X_train, y_train, X_test, y_test, fold_id=0):
    ...
    # Final test statistics and attention analysis
    analysis = collect_attention_and_probs(model, test_loader)
    preds_all, labels_all, probs_all = analysis['preds'], analysis['labels'], analysis['probs']
    ...
    return {
        ...
        'mask_examples': analysis['mask_examples'],
    }
...
def evaluate_under_missing_condition(model, test_loader, mode='full', ratio=0.0, region='frontal', block_size='small'):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            eval_mask_keep = build_eval_mask(
                b, h, w, mode=mode, ratio=ratio, region=region,
                block_size=block_size, device=inputs.device
            )
...
def train_subject(X, y, n_splits=10):
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(len(labels_np)), labels_np)):
        ...
        result = train_one_fold(X_train, y_train, X_test, y_test, fold_id=fold)

        # Evaluate under incomplete input conditions (Chapter 5 experiments)
        test_dataset = TensorDataset(torch.FloatTensor(X_test), torch.LongTensor(y_test.argmax(1)))
        ...
        print(f"Fold {fold + 1} -> Acc: {result['acc']:.4f}, F1: {result['f1']:.4f}, R50: {result['stability']['random_50']['acc']:.4f}, 16ch: {result['stability']['channels16']['acc']:.4f}, 8ch: {result['stability']['channels8']['acc']:.4f}")
    ...
    stability_keys = ['full','random_10','random_20','random_30','random_50',
                      'block_small','block_medium','block_large',
                      'region_frontal','region_central','region_parietal_occipital',
                      'channels8','channels16','channels24']
    stability_summary = {}
    for key in stability_keys:
        stability_summary[key] = {
            'acc': float(np.mean([r['stability'][key]['acc'] for r in fold_results])),
            'f1': float(np.mean([r['stability'][key]['f1'] for r in fold_results])),
        }

    all_preds = sum([r['preds'] for r in fold_results], [])
    all_labels = sum([r['labels'] for r in fold_results], [])
    all_probs = sum([r['probs'] for r in fold_results], [])
    temporal_all = np.vstack([r['temporal'] for r in fold_results if r['temporal'] is not None]) if any(r['temporal'] is not None for r in fold_results) else None
    band_mean = np.mean(np.stack([r['band'] for r in fold_results if r['band'] is not None], axis=0), axis=0) if any(r['band'] is not None for r in fold_results) else None
    spatial_mean = np.mean(np.stack([r['spatial'] for r in fold_results if r['spatial'] is not None], axis=0), axis=0) if any(r['spatial'] is not None for r in fold_results) else None
    mask_examples = []
    for r in fold_results:
        mask_examples.extend(r['mask_examples'])
...
if __name__ == '__main__':
    # TODO: Replace with your own path to the preprocessed DEAP data
    ...
    print(f'\n{"=" * 80}')
    print('RMA-Net (Chapter 5) on DEAP')
    ...
    if USE_SAVED_CACHE and (not FORCE_RETRAIN) and os.path.exists(FULL_CACHE_PATH):
        ...
        for sub in subject_list:
            ...
            print(f'\n{"#" * 80}')
            print(f'Training Subject {sub}')
            print(f'{"#" * 80}')

            X, y = load_data(sub, dataset_dir, flag=task_flag)
            ...
            if result['temporal'] is not None:
                state['all_temporal_weights'].append(result['temporal'])
            if result['band'] is not None:
                state['all_band_attention'].append(result['band'])
            if result['spatial'] is not None:
                state['all_spatial_maps'].append(result['spatial'])
            ...
            print(f"           Stability -> full={result['stability']['full']['acc']:.4f}, r20={result['stability']['random_20']['acc']:.4f}, r50={result['stability']['random_50']['acc']:.4f}, 24ch={result['stability']['channels24']['acc']:.4f}, 8ch={result['stability']['channels8']['acc']:.4f}")
    ...
    if len(state['all_stability']) > 0:
        keys = ['full','random_10','random_20','random_30','random_50',
                ...
                'channels8','channels16','channels24']
        print('-' * 80)
        print('Chapter-5 stability summary:')
        for key in keys:
            acc_mean = np.mean([x[key]['acc'] for x in state['all_stability']])
            f1_mean = np.mean([x[key]['f1'] for x in state['all_stability']])
            print(f'{key:>26s} -> Acc={acc_mean:.4f}, F1={f1_mean:.4f}')
    print('\n' + '=' * 50)
    ...
    if len(state['all_acc']) > 0 and len(state['all_f1']) > 0:
        plot_boxplot(state['all_acc'], state['all_f1'], os.path.join(SAVE_DIR, 'boxplot.png'))
    if len(state['all_losses_per_subject']) > 0:
    ...
    if len(state['all_confusion_matrices']) > 0:
        cm = np.sum(np.stack(state['all_confusion_matrices'], axis=0), axis=0).astype(int)
        plot_confusion_matrix_heatmap(cm, os.path.join(SAVE_DIR, 'confusion_matrix.png'), class_names=get_class_names(num_classes))
    ...
    if len(state['all_temporal_weights']) > 0:
        temporal = np.vstack(state['all_temporal_weights'])
        plot_temporal_attention_weights(temporal, os.path.join(SAVE_DIR, 'temporal_attention_weights.png'))
    if len(state['all_band_attention']) > 0:
        band_mean = np.mean(np.stack(state['all_band_attention'], axis=0), axis=0)
        plot_frequency_importance(band_mean, os.path.join(SAVE_DIR, 'frequency_importance.png'))
    if len(state['all_spatial_maps']) > 0:
        spatial_mean = np.mean(np.stack(state['all_spatial_maps'], axis=0), axis=0)
        plot_spatial_attention_map(spatial_mean, os.path.join(SAVE_DIR, 'spatial_attention_map.png'))
    if len(state['all_stability']) > 0:
        plot_stability_summary(state['all_stability'], os.path.join(SAVE_DIR, 'stability_summary.png'))
        print('\nStability summary (mean acc):')
        stable_keys = ['full','random_10','random_20','random_30','random_50','block_small','block_medium','block_large','region_frontal','region_central','region_parietal_occipital','channels8','channels16','channels24']
        for key in stable_keys:
            vals = [x[key]['acc'] for x in state['all_stability'] if key in x]
            if len(vals) > 0:
                print(f'  {key:<26} = {np.mean(vals):.4f}')
    if len(state['all_mask_examples']) > 0:
        plot_mask_examples(state['all_mask_examples'], os.path.join(SAVE_DIR, 'mask_examples.png'))
